...
def inventory():
    ...
    for i in sheets:
        if sheet_counter == len(sheets):
            ...
            cabinet = ws.cell(row=row, column=cabinet_column).value
            mmex = ws.cell(row=row, column=mmex_column).value

            # Prettify output - PrettyTable
            table = PrettyTable()
            table.field_names = ['Sheets', 'Memory', 'Cabinet', 'MMEX', 'VDR', 'ECC']
            table.add_row([sheets[sheet_counter], serial, cabinet, mmex, vdr, ecc])
            ...
            if args.add:
                if df.empty:
                    ...
                    if mmex - args.add < 0:
                        logger.error(f"\n\n\n{df}\nYou cannot do that.\n"
                                     f"While available quantity on MMEX is "
                                     f"{ws.cell(row=row, column=mmex_column).value}\n"
                    ...
                    ws.cell(row=row, column=mmex_column).value -= args.add
